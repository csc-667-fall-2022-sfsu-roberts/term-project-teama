...
const findGameIdByUserId = (userid) => {
    return db.any('SELECT "game_id" FROM "game_player" WHERE "player_id"=${userid}', {userid})
}
...
const userIsPlayingGame = (gameid, userid) => {
    let res = db.one('SELECT "player_id" FROM "game_player" WHERE "game_id"=${gameId} AND "player_id"=${userId}', {gameid, userid});
...
const  createNewGameUsers = async (gameid, userid, iscreator) => {
    ...
    return db.one('INSERT INTO "game_player" ("game_id", "player_id", "player_index") VALUES (${gameid}, ${userid}, ${playerIndex}) RETURNING id', {gameid, userid, playerIndex});
}
...
const findAllUsersByGameId = (gameid) => {
    return db.any('SELECT * FROM "game_player" WHERE "game_id"=${gameid} ORDER BY "player_index" ASC', {gameid})
}

const findUserByGameUserId = (gameid, userid) => {
    return db.oneOrNone('SELECT * FROM "game_player" WHERE "game_id"=${gameid} AND "player_id"=${userid}', {gameid, userid})
}

const deleteUserByGameUserId = (gameid, userid) => {
    return db.any('DELETE FROM "game_player" WHERE "game_id"=${gameid} AND "player_id"=${userid}', {gameid, userid})
}

const deleteALLUserByGameId = (gameid, userid) => {
    return db.any('DELETE FROM "game_player" WHERE "game_id"=${gameid}', {gameid, userid})
}
...
const findNumOfUsersByGameId = (gameid) => {
    return db.one('SELECT COUNT(*) FROM "game_player" WHERE "game_id"=${gameid}', {gameid})
}
const findEngagedGames = async (userid) => {
    return db.any('SELECT * FROM "game" WHERE "id" IN (SELECT "game_id" FROM "game_player" WHERE "player_id"=${userid}) ORDER BY "id" DESC', {userid})
}

const findNotEngagedGames = (userid) => {
    return db.any('SELECT * FROM "game" WHERE "id" NOT IN (SELECT "game_id" FROM "game_player" WHERE "player_id"=${userid}) ORDER BY "id" DESC', {userid})
}
